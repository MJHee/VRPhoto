//
//  MBScrollNavigationController.swift
//  VR
//
...
@objc public protocol MBScrollNavigationControllerDelegate: NSObjectProtocol {
    
    /// Called when the state of the navigation bar changes
    @objc optional func mbScrollNavigationController(_ controller: MBScrollNavigationController, didChangeState state: NavigationBarState)
    
    /// Called when the state of the navigation bar is about to change
    @objc optional func mbScrollNavigationController(_ controller: MBScrollNavigationController, willChangeState state: NavigationBarState)
}

open class MBScrollNavigationController: UINavigationController, UIGestureRecognizerDelegate {

    open fileprivate(set) var state: NavigationBarState = .expanded {
        willSet {
            if state != newValue {
                scrollNavigationBarDelegate?.mbScrollNavigationController?(self, willChangeState: newValue)
            }
        }
        didSet {
            navigationBar.isUserInteractionEnabled = (state == .expanded)
            if state != oldValue {
                scrollNavigationBarDelegate?.mbScrollNavigationController?(self, didChangeState: state)
            }
        }
    }
    
    /// The delegate for the scrolling navbar controller
    open weak var scrollNavigationBarDelegate: MBScrollNavigationControllerDelegate?
    ...
    open func followScrollView(_ scrollableView: UIView, delay: Double = 0, scrollSpeedFactor: Double = 1, followers:[UIView] = []) {
        ...
        gestureRecongnizer = UIPanGestureRecognizer(target: self, action: #selector(MBScrollNavigationController.handlePan(_:)))
        ...
        //通知
        NotificationCenter.default.addObserver(self, selector: #selector(MBScrollNavigationController.didBecomeActive(_:)), name: NSNotification.Name.UIApplicationDidBecomeActive, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(MBScrollNavigationController.didRotate(_:)), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
    ...
    fileprivate func scrollWithDelta(_ delta: CGFloat) {
        ...
        if scrollDelta < 0 {
            ...
            if
            delayDistance > 0 && maxDelay < contentOffset.y {
                return
            }
    ...
    fileprivate func updateNavigationBarAlpha() {
        ...
        //change the alpha channel of every item on the navbar
        let frame = navigationBar.frame
        let alpha = (frame.origin.y +  deltaLimit)
 / frame.size.height
        ...
        navigationBar.subviews.filter(shouldHideView).forEach{ $0.alpha = alpha }
    ...
    fileprivate func updateContentOffset(_ delta: CGFloat) {
        ...
        if let scrollView = scrollView() {
            scrollView.setContentOffset(CGPoint(x: contentOffset.x, y: contentOffset.y - delta), animated: true)
        }
    }
